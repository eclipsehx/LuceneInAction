...
public class QueryParserTest extends TestCase {
	...
	public void testPrefixQuery() throws Exception {
		...
		QueryParser parser = new QueryParser(Version.LUCENE_30, "category", new StandardAnalyzer(Version.LUCENE_30));
	...
	public void testGrouping() throws Exception {
		...
		Query query = new QueryParser(Version.LUCENE_30, "subject", analyzer).parse("(agile OR extreme) AND methodology");
	...
	public void testTermRangeQuery() throws Exception {
		...
		Query query = new QueryParser(Version.LUCENE_30, "subject", analyzer).parse("title2:[Q TO V]"); 	// A
		assertTrue(query instanceof TermRangeQuery);

		TopDocs matches = searcher.search(query, 10);
		assertTrue(TestUtil.hitsIncludeTitle(searcher, matches, "Tapestry in Action"));

		query = new QueryParser(Version.LUCENE_30, "subject", analyzer).parse("title2:{Q TO \"Tapestry in Action\"}"); 	// B
	...
	public void testPhraseQuery() throws Exception {
		...
		Query q = new QueryParser(Version.LUCENE_30, "field", new StandardAnalyzer(Version.LUCENE_30)).parse("\"This is Some Phrase*\"");
	...
	public void testWildcard() {
		try {
			new QueryParser(Version.LUCENE_30, "field", analyzer).parse("*xyz");
	...
	public void testBoost() throws Exception {
		Query q = new QueryParser(Version.LUCENE_30, "field", analyzer).parse("term^2");
		assertEquals("term^2.0", q.toString("field"));
	}

	public void testParseException() {
		try {
			new QueryParser(Version.LUCENE_30, "contents", analyzer).parse("^&#");
		} catch (ParseException expected) {
