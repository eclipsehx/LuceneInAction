...
public class FunctionQueryTest extends TestCase {
	 ...
	 * #2 Doc 1 is first because its static score (42) is higher than doc 0's (7)
	 */
	...
	/*
	 * #1 Even though document 0 is a better match to the original query,
	 * document 1 gets a better score after multiplying in its score field
	 */
	...
	static class RecencyBoostingQuery extends CustomScoreQuery {
		...
		private class RecencyBooster extends CustomScoreProvider {
			...
			public RecencyBooster(IndexReader r) throws IOException {
				super(r);
				publishDay = FieldCache.DEFAULT.getInts(r, dayField); // #A
			}
			...
			public float customScore(int doc, float subQueryScore, float valSrcScore) {
				...
				if (daysAgo < maxDaysAgo) { // #C
					float boost = (float) (multiplier * (maxDaysAgo - daysAgo) / maxDaysAgo); // #D
	...
	public void testRecency() throws Throwable {
		...
		for (int i = 0; i < hits.scoreDocs.length; i++) {
			...
			System.out.println((1 + i) + ": " + doc.get("title")
					+ ": pubmonth=" + doc.get("pubmonth") + " score="
