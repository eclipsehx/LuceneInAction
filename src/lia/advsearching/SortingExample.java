...
public class SortingExample {
	...
	public void displayResults(Query query, Sort sort) throws IOException {		// #1

		IndexSearcher searcher = new IndexSearcher(directory);

		searcher.setDefaultFieldSortScoring(true, false);	// #2

		TopDocs results = searcher.search(query, null, 20, sort);	// #3
...
//			out.println(searcher.explain(query, docID));	// #7
	...
	public static void main(String[] args) throws Exception {
		...
		QueryParser parser = new QueryParser(Version.LUCENE_30, "contents", new StandardAnalyzer(Version.LUCENE_30));	// #1
		...
		example.displayResults(query, new Sort(new SortField("category", SortField.STRING), SortField.FIELD_SCORE, new SortField("pubmonth", SortField.INT, true)));
