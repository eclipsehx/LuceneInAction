...
public class SpatialLuceneExample {
	...
	private void addLocation(IndexWriter writer, String name, double lat, double lng) throws IOException {
		...
		for (; startTier <= endTier; startTier++) {
			...
			CartesianTierPlotter ctp = new CartesianTierPlotter(startTier, projector, tierPrefix);							// #D
	...
	public void findNear(String what, double latitude, double longitude, double radius) throws CorruptIndexException, IOException {
		...
		DistanceQueryBuilder dq = new DistanceQueryBuilder(latitude, longitude, radius, latField, lngField, tierPrefix, true);		// #A

		Query tq = (what == null ? new TermQuery(new Term("metafile", "doc")) : new TermQuery(new Term("name", what)));				// #B

		DistanceFieldComparatorSource dsort = new DistanceFieldComparatorSource(dq.getDistanceFilter());							// #C
		Sort sort = new Sort(new SortField("foo", dsort));																			// #C

		TopDocs hits = searcher.search(tq, dq.getFilter(), 10, sort);

		Map<Integer, Double> distances = dq.getDistanceFilter().getDistances();														// #D
	 ...
	 * #B Match all documents --> TermQuery(new Term("metafile", "doc"))
