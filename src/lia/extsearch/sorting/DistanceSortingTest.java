...
public class DistanceSortingTest extends TestCase {
	...
	public void testNeareastRestaurantToWork() throws Exception {
		...
		TopFieldDocs docs = searcher.search(query, null, 3, sort);	// #1 Specify maximum hits returned

		assertEquals(4, docs.totalHits);			// #2 Total number of hits
		assertEquals(3, docs.scoreDocs.length);		// #3 Return total number of documents

		FieldDoc fieldDoc = (FieldDoc) docs.scoreDocs[0];	// #4 Get sorting values

		assertEquals("(10,10) -> (9,6) = sqrt(17)", new Float(Math.sqrt(17)), fieldDoc.fields[0]);	// #5 Give value of first computation
